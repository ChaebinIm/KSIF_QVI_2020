...
class CriticNetwork(Module):
    ...
    def forward(self, src: Tensor, action_src: Tensor, src_mask: Optional[Tensor] = None,
        ...
        #Run through Transformer Encoder network
        ...
        output = self.CAAN(memory, action_src, memory_mask=memory_mask, memory_key_padding_mask=memory_key_padding_mask,
...
class CAAN(Module):
    ...
    def forward(self, memory: Tensor, action_mem: Tensor, threshold: int,
        ...
        for mod in self.layers:
            output = mod(memory, action_mem, threshold=threshold, key_table=key_table)
        return output
...
class TransformerEncoderLayer(Module):
    ...
    def forward(self, src: Tensor, src_mask: Optional[Tensor] = None,
        ...
        src2 = self.self_attn(src, src, src, attn_mask=src_mask,
                             key_padding_mask=src_key_padding_mask)[0]  # [period, company_num, column_num]
...
class CAAN_Layer(Module):
    def __init__(self, d_model, nhead2, dim_feedforward=2, dropout=0.1, activation="relu", column_num = None):
        ...
        self.linear_act = Linear(1, self.num_hidden_node2)
        self.linear_q = Linear(self.num_hidden_node2, 1)
        self.relu = torch.relu
        self.norm1 = LayerNorm(self.num_hidden_node1)
        self.norm2 = LayerNorm(self.num_hidden_node2)
        self.dropout = Dropout(dropout)
    ...
    def forward(self, memory: Tensor, action_mem: Tensor, threshold: int = 0, key_table=pd.Series(),
                memory_mask: Optional[Tensor] = None,
        ...
        # tgt.shape
        action_mem = torch.as_tensor(action_mem, dtype=torch.double, device='cpu')
        action_value = self.linear_act(action_mem)
        # action_value.shape
        tgt = torch.add(tgt, action_value)
        # tgt.shape
        tgt = self.relu(tgt)
        # tgt.shape
        q_value = self.linear_q(tgt)
